...
백엔드만 올라와 있습니다. 지금 GitHub 상태 그대로 두 저장소를 받아 돌리면
아래가 화면에서 동작하지 않습니다. 프론트 담당자에게 push를 요청하세요.
...
프론트 저장소(`RISE-client`, `feature/FE-backend-integration`)에 **push되지 않은 커밋 6개**가
있습니다. 백엔드가 아무리 정확해도 이게 없으면 화면에서 동작하지 않습니다.
...
| `c424279` 로그아웃 토큰 삭제 + 그룹원 프로필 | 로그아웃해도 JWT가 기기에 남음(보안), 그룹원 프로필에 미션 결과 없음 |

### 그 외 프론트 작업

1. **배포된 백엔드 주소로 `.env` 수정 후 재빌드** — `VITE_API_BASE_URL`이 로컬 주소면
   배포된 앱이 서버를 전혀 못 찾습니다. 배포 후 반드시.
2. **`GET /api/groups/me/members/{userId}` 연동 완료됨** (커밋 `c424279`) — 추가 작업 없음.
3. **미션 개수가 3개 고정이라고 가정한 UI가 있는지 확인** — 난이도 상승 시 4개,
   3일 연속 실패 시 1개가 내려갑니다.
4. **`main` 브랜치 병합** — 대회 심사는 main/master 기준입니다.
