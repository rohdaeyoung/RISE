...
### 16. 반응·댓글은 아직 내 기기에만 저장됩니다 ⚠️

그룹 피드의 반응(❤️👍😂😮😢)과 댓글이 `AppContext`에만 쌓입니다. 그래서 **남긴 사람 화면에만
보이고 그룹원에게는 안 보입니다.** 서로의 진행을 보며 동기부여를 받는 게 목적이라 반쪽짜리입니다.

백엔드에 API를 만들어 뒀으니 연결해주세요. 셋 다 **갱신된 피드 전체**를 돌려줍니다.
...
- 반응은 **같은 이모지를 다시 보내면 취소, 다른 이모지를 보내면 교체**됩니다.
  지금 `TOGGLE_REACTION` 리듀서와 같은 규칙이라 로직을 바꿀 필요는 없습니다.
- 댓글 작성자 표시는 `authorNickname`과 `me`로 만드세요. 서버는 `'나'`/`'그룹원'` 같은
  화면용 문구를 만들지 않습니다.
- 모두 **오늘 기준**입니다. 날짜가 바뀌면 새 피드가 되므로 로컬에서 따로 비울 필요가 없습니다.

### 17. 의상을 추가·교체하면 백엔드 카탈로그도 같이 바꿔야 합니다

캐릭터 아트를 `formal/picnic/sport`에서 `sailor/coat/detective`로 교체했을 때, 백엔드
카탈로그가 그대로라 **새 의상을 사려 하면 `SHOP_001 존재하지 않는 의상`으로 거절**됐습니다.
파자마는 프론트 30코인 / 백엔드 35코인으로 가격도 어긋나 코인이 맞지 않았습니다.

`ShopPage.jsx`의 `OUTFIT_SETS`와 백엔드 `OutfitCatalog.ITEMS`는 **id와 가격이 항상 같아야
합니다.** 의상을 건드리면 백엔드 담당자에게 알려주세요.

> 예전 의상을 입고 있던 사람은 백엔드가 기본 의상으로 바꿔 내려보냅니다.
> 프론트에 그 이미지가 없어 캐릭터가 깨지는 것을 막기 위해서입니다.
