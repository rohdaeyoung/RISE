...
### 배포 후 실사용에서 잡은 버그 (2026-08-13)

배포된 웹앱을 실제 폰으로 쓰면서 나온 것들입니다. **전부 자동화 테스트로는 안 잡히는 유형**이라,
아래 세 가지는 앞으로도 사람이 직접 써봐야 발견됩니다.
...
### 2. 상태를 바꾸는 화면에 API 호출이 빠지지 않았는지 점검

`GroupSettingsPage`에서 그룹 나가기와 방 설정 저장이 **서버 호출 없이 화면 상태만 바꾸고 있었습니다.**
같은 유형이 더 없는지 확인이 필요합니다. 점검 방법은 간단합니다 —
`dispatch({ type: ... })`를 검색해서, 서버에 남아야 하는 값인데 옆에 API 호출이 없으면 그게 버그입니다.
