...
frontend/src
  ...
  ├── pages/        화면 단위 컴포넌트
  ├── components/   재사용 UI (캐릭터, 미션 카드, 결과 시트, 댓글 시트 등)
  └── utils/        이미지 리사이즈 등
```
...
받아오는데 식단만 빠져 있었기 때문입니다. 그룹 피드의 반응·댓글도 같은 이유로 `sync()`에
포함돼 있습니다. 새 데이터를 서버에 저장하기 시작했다면 `AppContext.jsx`의 `sync()`에도
조회를 추가하세요.
...
위 "1순위/2순위"였던 반응·댓글 연동과 오류 처리는 **완료**되어 "최근 수정 (2026-08-15)"로 옮겼습니다.
아래는 급하지 않아 미룬 것들입니다.
