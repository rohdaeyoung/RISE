...
### 10. `internalFit`은 화면에 띄우지 마세요 — 단 `recognized`는 띄웁니다

식단 분석 결과의 목표적합도(`good`/`normal`/`bad`)는 **다음 미션 난이도를 정하는 내부 값**입니다.
기획상 사용자에게는 "달성 / 미달성"만 보여줍니다. (애초에 서버가 응답에 담지 않습니다.)

반대로 `recognized`는 **반드시 보여주세요.** AI가 사진에서 무엇을 봤는지(예: `"단백질 음료"`)를
서버가 판정과 함께 내려주는 값입니다. 이게 없으면 사용자는 AI가 사진을 실제로 읽었는지 알 수 없고,
거절당했을 때 무엇을 다시 찍어야 할지도 알 수 없습니다.

- 식단: 분석 응답의 `data.recognized`
- 생활습관: 인증 성공 응답의 `data.recognized`, 거절 시에는 `MISSION_004` 메시지에 함께 들어옵니다
- **목록을 다시 불러올 때는 `null`입니다** — 방금 판정한 응답에만 담깁니다. 없으면 그리지 마세요.

### 10-1. 사진은 원본 그대로 올리지 마세요

폰 카메라 원본은 4032x3024·3MB 안팎입니다. 그대로 올리면 현장 네트워크에서 업로드에만 몇 초가 걸리고,
서버의 multipart 상한(10MB)에 걸리는 기종도 있습니다. `toUploadFile()`로 긴 변 1280px JPEG으로 줄여서
보냅니다 — 서버도 저장 전에 1024px로 줄이고, AI 판정 정확도도 달라지지 않습니다.

캔버스로 다시 그리는 과정에서 **EXIF 회전값이 픽셀에 반영**되므로, 폰을 세워서 찍은 사진이 누운 채로
판정되는 일도 함께 없어집니다. 브라우저가 사진을 디코딩하지 못하면 원본을 그대로 올립니다
(인증을 아예 못 하는 것보다 낫습니다).
